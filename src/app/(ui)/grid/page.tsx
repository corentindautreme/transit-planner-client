export default function Page() {
    return (
        <div className="h-full bg-white rounded-xl py-3 px-1 overflow-scroll">
            <div className="grid grid-cols-[repeat(20,_40px)] grid-rows-[repeat(20,_40px)_30px_40px_30px_repeat(5,_40px)]">

                {/* River */}
                <div className="row-start-22 row-end-23 col-start-8 col-end-17 bg-sky-300"></div>

                {/* Main track */}
                <div className="row-start-18 row-end-19 col-start-3 col-end-4 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                {/* Bottom fork to 2-fork subfork */}
                <div className="row-start-18 row-end-19 col-start-4 col-end-5 flex flex-col">
                    <div className="h-[calc(50%-0.125rem)]"></div>
                    <div className="relative flex flex-1">
                        <div className="absolute w-full border-t-4 border-yellow-500"></div>
                        <div className="w-[calc(50%+0.125rem)] h-full border-t-4 border-e-4 border-yellow-500 rounded-tr-2xl"></div>
                        <div className="w-[calc(50%+0.125rem)] h-full -ms-1 border-s-4 border-t-4 border-yellow-500 rounded-tl-2xl"></div>
                    </div>
                </div>

                <div className="row-start-19 row-end-20 col-start-4 col-end-5 flex justify-center">
                    <div className="w-1 h-full bg-yellow-500"></div>
                </div>

                {/* subfork - entrance */}
                <div className="row-start-20 row-end-21 col-start-2 col-end-3 flex flex-col items-end">
                    <div className="flex-1"></div>
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-s-4 border-t-4 border-yellow-500 rounded-tl-xl"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-3 col-end-4 flex items-center">
                    <div className="w-full h-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-4 col-end-5 flex flex-col">
                    <div className="flex flex-1">
                        <div className="w-[calc(50%+0.125rem)] h-full border-b-4 border-r-4 border-yellow-500 rounded-br-xl"></div>
                        <div className="w-[calc(50%+0.125rem)] h-full -ms-1 border-s-4 border-b-4 border-yellow-500 rounded-bl-xl"></div>
                    </div>
                    <div className="h-[calc(50%-0.125rem)]"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-5 col-end-6 flex items-center">
                    <div className="w-full h-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-6 col-end-7 flex flex-col">
                    <div className="flex-1"></div>
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-t-4 border-e-4 border-yellow-500 rounded-tr-xl"></div>
                </div>
                {/* end */}

                {/* subfork - left branch */}
                <div className="row-start-21 row-end-22 col-start-2 col-end-3 flex justify-center">
                    <div className="w-1 h-full bg-yellow-500"></div>
                </div>
                {/* end */}

                {/* subfork - right branch */}
                <div className="row-start-21 row-end-22 col-start-6 col-end-7 flex justify-center">
                    <div className="w-1 h-full bg-yellow-500"></div>
                </div>
                {/* end */}

                {/* subfork - closure with station */}
                <div className="row-start-22 row-end-23 col-start-2 col-end-3 flex flex-col items-end">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-s-4 border-b-4 border-yellow-500 rounded-bl-xl"></div>
                </div>

                <div className="row-start-22 row-end-23 col-start-3 col-end-4 flex items-center">
                    <div className="w-full h-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-22 row-end-23 col-start-4 col-end-5 flex flex-col gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-end justify-center">
                        <div className="flex items-center gap-1 justify-center">
                            {/*<div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">1</div>*/}
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center">
                        <div className="w-full min-w-max text-ellipsis text-end md:text-center bg-yellow-500 px-1 py-0.5 font-bold rounded-lg">Stanica</div>
                    </div>
                </div>

                <div className="row-start-22 row-end-23 col-start-5 col-end-6 flex items-center">
                    <div className="w-full h-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-22 row-end-23 col-start-6 col-end-7 flex flex-col">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-b-4 border-e-4 border-yellow-500 rounded-br-xl"></div>
                </div>
                {/* end */}

                {/* end */}

                <div className="row-start-18 row-end-19 col-start-5 col-end-6 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-18 row-end-19 col-start-6 col-end-7 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-end md:text-center">Stanica</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">1</div>
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">1</div>
                        </div>
                    </div>
                </div>

                <div className="row-start-18 row-end-19 col-start-7 col-end-8 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>
                {/* end */}

                {/* Trolleybus - upper shared route */}
                <div className="row-start-14 row-end-15 col-start-11 col-end-12 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-red-500 rounded-b-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-red-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center">Stanica</div>
                    </div>
                </div>

                <div className="row-start-15 row-end-19 col-start-11 col-end-12 flex justify-center">
                    <div className="h-full w-1 bg-red-500"></div>
                </div>

                <div className="row-start-19 row-end-20 col-start-11 col-end-12 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-red-500 rounded-b-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-red-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center">Stanica</div>
                    </div>
                </div>

                <div className="row-start-20 row-end-21 col-start-11 col-end-12 flex flex-col">
                    <div className="h-[calc(50%+0.125rem)] flex justify-center">
                        <div className="w-1 h-full bg-red-500"></div>
                    </div>
                    <div className="grow flex justify-center">
                        <div className="w-1 h-full bg-red-500"></div>
                    </div>
                </div>

                <div className="row-start-21 row-end-23 col-start-11 col-end-12 flex justify-center">
                    <div className="h-full w-1 bg-red-500"></div>
                </div>

                <div className="row-start-22 row-end-23 col-start-11 col-end-12 flex justify-center">
                    <div className="h-full w-1 bg-white"></div>
                    <div className="h-full w-1 bg-red-500"></div>
                    <div className="h-full w-1 bg-white"></div>
                </div>

                <div className="row-start-23 row-end-24 col-start-11 col-end-12 flex justify-center">
                    <div className="h-full w-1 bg-red-500"></div>
                </div>

                <div className="row-start-24 row-end-25 col-start-11 col-end-12 flex flex-col">
                    <div className="relative flex flex-1">
                        <div className="absolute bottom-0 w-full border-b-4 border-red-500"></div>
                        <div className="w-[calc(50%+0.125rem)] h-full border-b-4 border-e-4 border-red-500 rounded-br-2xl"></div>
                        <div className="w-[calc(50%+0.125rem)] h-full -ms-1 border-s-4 border-b-4 border-red-500 rounded-bl-2xl"></div>
                    </div>
                    <div className="h-[calc(50%-0.125rem)]"></div>
                </div>
                {/* end */}

                {/* 2-track fork - entrance */}
                <div className="row-start-16 row-end-17 col-start-8 col-end-9 flex flex-col items-end justify-end">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-s-4 border-t-4 border-yellow-500 rounded-tl-xl"></div>
                </div>

                <div className="row-start-17 row-end-18 col-start-8 col-end-9 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-18 row-end-19 col-start-8 col-end-9 flex flex-col">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-e-4 border-b-4 border-yellow-500 rounded-br-xl"></div>
                    <div className="-mt-1 w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-t-4 border-e-4 border-yellow-500 rounded-tr-xl"></div>
                </div>

                <div className="row-start-19 row-end-20 col-start-8 col-end-9 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-8 col-end-9 flex flex-col items-end">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-l-4 border-b-4 border-yellow-500 rounded-bl-xl"></div>
                </div>
                {/* end */}

                {/* Fork - top branch */}
                <div className="row-start-16 row-end-17 col-start-11 col-end-12 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-16 row-end-17 col-start-12 col-end-13 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Park</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                        </div>
                    </div>
                </div>

                <div className="row-start-16 row-end-17 col-start-13 col-end-15 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-16 row-end-17 col-start-15 col-end-16 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Banka</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                        </div>
                    </div>
                </div>

                <div className="row-start-16 row-end-17 col-start-16 col-end-18 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-16 row-end-17 col-start-18 col-end-19 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Katedrala</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                        </div>
                    </div>
                </div>

                <div className="row-start-16 row-end-17 col-start-19 col-end-20 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>
                {/* end */}

                {/* Fork - bottom branch */}
                <div className="row-start-20 row-end-21 col-start-9 col-end-12 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-12 col-end-13 flex flex-col items-center justify-center">
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center justify-end">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center">
                        <div className="w-full min-w-max text-ellipsis text-center">Skenderija</div>
                    </div>
                </div>

                <div className="row-start-20 row-end-21 col-start-13 col-end-14 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Pošta</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                </div>

                <div className="row-start-20 row-end-21 col-start-14 col-end-15 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-15 col-end-16 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Drvenija</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                </div>

                <div className="row-start-20 row-end-21 col-start-16 col-end-17 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-17 col-end-18 flex flex-col items-center justify-center">
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center">
                        <div className="w-full min-w-max text-ellipsis text-center">Latinska ćuprija</div>
                    </div>
                </div>

                <div className="row-start-20 row-end-21 col-start-18 col-end-19 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-19 col-end-20 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Vijećnica</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-yellow-500 rounded-e-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-yellow-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                </div>
                {/* end */}

                {/* 2-track connector with station */}
                <div className="row-start-16 row-end-17 col-start-10 col-end-11 relative flex flex-col justify-end">
                    <div className="absolute top-1/2 translate-y-[-0.125rem] w-full border-t-4 border-yellow-500"></div>
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-t-4 border-e-4 border-yellow-500 rounded-tr-xl"></div>
                </div>

                <div className="row-start-17 row-end-18 col-start-10 col-end-11 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-18 row-end-19 col-start-10 col-end-11 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">6</div>
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-yellow-500 rounded-b-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-yellow-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center bg-yellow-500 px-1 py-0.5 font-bold rounded-lg">Skenderija</div>
                    </div>
                </div>

                <div className="row-start-19 row-end-20 col-start-10 col-end-11 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-10 col-end-11 relative flex flex-col">
                    <div className="absolute top-1/2 -translate-y-0.5 w-full border-b-4 border-yellow-500"></div>
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-e-4 border-b-4 border-yellow-500 rounded-br-xl"></div>
                </div>
                {/* end */}

                <div className="row-start-16 row-end-17 col-start-13 col-end-14 flex items-center">
                    <div className="h-1 w-full bg-yellow-500"></div>
                </div>

                {/* 2-track end connector with station */}
                <div className="row-start-16 row-end-17 col-start-20 col-end-21 flex flex-col justify-end">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-t-4 border-e-4 border-yellow-500 rounded-tr-xl"></div>
                </div>

                <div className="row-start-17 row-end-18 col-start-20 col-end-21 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-18 row-end-19 col-start-20 col-end-21 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">1</div>
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">2</div>
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">3</div>
                            <div className="shrink-0 w-5 text-center rounded bg-yellow-500 text-lg font-bold">5</div>
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-yellow-500 rounded-b-xl"></div>
                        <div className="size-2 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-yellow-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center bg-yellow-500 px-1 py-0.5 font-bold rounded-lg">Baščaršija</div>
                    </div>
                </div>

                <div className="row-start-19 row-end-20 col-start-20 col-end-21 relative flex justify-center">
                    <div className="w-1 bg-yellow-500"></div>
                </div>

                <div className="row-start-20 row-end-21 col-start-20 col-end-21 flex flex-col">
                    <div className="w-[calc(50%+0.125rem)] h-[calc(50%+0.125rem)] border-e-4 border-b-4 border-yellow-500 rounded-br-xl"></div>
                </div>
                {/* end */}

                {/* Trolleybus - bottom route */}
                <div className="row-start-24 row-end-25 col-start-12 col-end-13 flex items-center">
                    <div className="h-1 w-full bg-red-500"></div>
                </div>
                
                <div className="row-start-24 row-end-25 col-start-13 col-end-14 flex items-center">
                    <div className="h-1 w-full bg-red-500"></div>
                </div>

                <div className="row-start-24 row-end-25 col-start-14 col-end-15 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center">Stanica</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-red-500 rounded-e-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 h-1 bg-red-500 rounded-s-xl"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                        </div>
                    </div>
                </div>

                <div className="row-start-24 row-end-25 col-start-15 col-end-16 flex items-center">
                    <div className="h-1 w-full bg-red-500"></div>
                </div>

                <div className="row-start-24 row-end-25 col-start-16 col-end-17 flex flex-col items-center justify-center">
                    {/* Station name */}
                    <div className="min-h-0 flex-1/2 flex flex-col items-center justify-end">
                        <div className="w-full min-w-max text-ellipsis text-center bg-red-500 px-1 py-0.5 mb-1 font-bold rounded-lg">Stanica</div>
                    </div>
                    {/* Marker */}
                    <div className="w-full flex flex-row items-center gap-0.5">
                        <div className="flex-1 h-1 bg-red-500 rounded-e-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1"></div>
                    </div>
                    {/* Station signs */}
                    <div className="min-h-0 flex-1/2 flex items-start justify-center">
                        <div className="flex items-center gap-1 justify-center mt-1">
                            <div className="shrink-0 w-5 text-center rounded bg-red-500 text-lg font-bold">1</div>
                            <div className="shrink-0 w-5 text-center rounded bg-red-500 text-lg font-bold">1</div>
                        </div>
                    </div>
                </div>
                {/* end */}

                {/* Trolleybus - right fork */}
                <div className="row-start-8 row-end-9 col-start-15 col-end-16 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                            <div className="shrink-0 w-10 text-center rounded bg-red-500 text-lg font-bold">105</div>
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-red-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center bg-red-500 px-1 py-0.5 font-bold rounded-lg">Jezero</div>
                    </div>
                </div>

                <div className="row-start-9 row-end-10 col-start-15 col-end-16 flex justify-center">
                    <div className="h-full w-1 bg-red-500"></div>
                </div>

                <div className="row-start-10 row-end-11 col-start-15 col-end-16 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-red-500 rounded-b-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-red-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center">Stanica</div>
                    </div>
                </div>

                <div className="row-start-11 row-end-12 col-start-15 col-end-16 flex justify-center">
                    <div className="h-full w-1 bg-red-500"></div>
                </div>

                <div className="row-start-12 row-end-13 col-start-15 col-end-16 flex gap-1 items-center justify-center">
                    {/* Station signs */}
                    <div className="min-w-0 flex-1/2 flex justify-end">
                        <div className="flex items-center gap-1 justify-center">
                        </div>
                    </div>
                    {/* Marker */}
                    <div className="h-full flex flex-col items-center gap-0.5">
                        <div className="flex-1 w-1 bg-red-500 rounded-b-xl"></div>
                        <div className="size-2 bg-red-500 rounded-full"></div>
                        <div className="flex-1 w-1 bg-red-500 rounded-t-xl"></div>
                    </div>
                    {/* Station name */}
                    <div className="min-w-0 flex-1/2 flex">
                        <div className="w-full min-w-max text-center">Stanica</div>
                    </div>
                </div>
                {/* end */}

                {/* Trolleybus - fork entrance */}
                <div className="row-start-13 row-end-14 col-start-11 col-end-12 flex">
                    <div className="grow"></div>
                    <div className="h-[calc(50%+0.125rem)] w-[calc(50%+0.125rem)] border-s-4 border-b-4 border-red-500 rounded-bl-xl"></div>
                </div>

                <div className="row-start-13 row-end-14 col-start-12 col-end-13 flex items-center">
                    <div className="w-full h-1 bg-red-500"></div>
                </div>

                <div className="row-start-13 row-end-14 col-start-13 col-end-14 flex flex-col">
                    <div className="grow"></div>
                    <div className="flex h-[calc(50%+0.125rem)]">
                        <div className="w-[calc(50%+0.125rem)] border-t-4 border-e-4 border-red-500 rounded-tr-xl"></div>
                        <div className="-ms-1 w-[calc(50%+0.125rem)] border-s-4 border-t-4 border-red-500 rounded-tl-xl"></div>
                    </div>
                </div>

                <div className="row-start-13 row-end-14 col-start-14 col-end-15 flex items-center">
                    <div className="w-full h-1 bg-red-500"></div>
                </div>

                <div className="row-start-13 row-end-14 col-start-15 col-end-16 flex">
                    <div className="h-[calc(50%+0.125rem)] w-[calc(50%+0.125rem)] border-b-4 border-e-4 border-red-500 rounded-br-xl"></div>
                </div>
                {/* end */}

            </div>
        </div>
    );
}